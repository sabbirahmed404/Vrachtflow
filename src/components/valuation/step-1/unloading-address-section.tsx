...
export function UnloadingAddressSection() {
  const [skipUnloading, setSkipUnloading] = useState(false);
  ...
  return (
    <FormSection
      title="Unloading Address (Drop-off Location)"
      showDivider={false}
    >
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-3 p-3 border border-border rounded-lg bg-muted/30">
          <Checkbox
            id="skipUnloading"
            checked={skipUnloading}
            onCheckedChange={(checked) => setSkipUnloading(checked as boolean)}
          />
          <div className="flex items-center gap-2">
            <SkipForward className="h-4 w-4 text-muted-foreground" />
            <Label
              htmlFor="skipUnloading"
              className="text-sm font-medium cursor-pointer"
            >
              Unloading address not known yet (skip for now)
            </Label>
          </div>
        </div>
        ...
        {!skipUnloading && (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                <Label
                  htmlFor="unloadHouseNumber"
                  className="text-sm font-medium text-foreground"
                >
                  House Number
                </Label>
                <Input
                  id="unloadHouseNumber"
                  value={houseNumber}
                ...
                <div className="flex items-center space-x-3 p-3 border border-border rounded-lg bg-card">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setHasLift(checked as boolean)
                    }
                  />
                  <div className="flex items-center gap-2">
                    <Building className="h-4 w-4 text-muted-foreground" />
                    <Label
                      htmlFor="unloadHasLift"
                      className="text-sm font-medium cursor-pointer"
                    >
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="unloadAccessNotes"
                placeholder='Any special instructions (e.g., "no elevator", "need traffic signs", gate codes)'
                rows={3}
